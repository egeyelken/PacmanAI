...
def cornersHeuristic(state, problem):
    ...
    for i in range(len(problem.corners)):
        if state[1][i] == False:
            mDistance = util.manhattanDistance(state[0], problem.corners[i])
            if mDistance > heuristic:
                heuristic = mDistance
    return heuristic
...
def foodHeuristic(state, problem):
    ...
    for food in foodGrid.asList():
        fHeuristic = mazeDistance(food, position, problem.startingGameState)
        if fHeuristic > heuristic:
