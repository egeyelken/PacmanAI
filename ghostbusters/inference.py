...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for key in self.beliefs.keys():
            self.beliefs[key] *= self.getObservationProb(observation, pacmanPos, key, jailPos)
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.allPositions:
            ...
            for k in self.allPositions:
                dst = positionDistribution[k] * self.beliefs[pos]
                helper[k] += dst
        for k in self.allPositions:
            self.beliefs[k] = helper[k]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            dist[p] += self.getObservationProb(observation, pacmanPos, p, jailPos)
    ...
    def elapseTime(self, gameState):
        ...
        for sample in particles:
            self.particles.append(self.getPositionDistribution(gameState, sample).sample())
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                probability *= self.getObservationProb(observation[i], pacmanPos, p[i], self.getJailPosition(i))
            dist[p] += probability
