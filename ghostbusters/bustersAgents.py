...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        likelyPositions = dict()
        i = 0
        for ghost in livingGhostPositionDistributions:
            likelyPositions[i] = ghost.argMax()
            i += 1
        action = None
        maxDist = 1000000
        for act in legal:
            for k in likelyPositions.keys():
                dist = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, act), likelyPositions[k])
                if dist < maxDist:
                    maxDist = dist
                    action = act
        return action
